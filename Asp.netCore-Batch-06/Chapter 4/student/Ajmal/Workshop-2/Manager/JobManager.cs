...
namespace CompanyMember.Manager
{
	public class JobManager : IMenu
	{
		...
		IApplicationRepository application = new ApplicationRepository();
		...
		private void ShowProviderMenu()
		{
			...
			Console.WriteLine("2. List all Jobs & Apply Job ");
			...
			switch (choice)
			{
				...
				case "2":
					...
					break;
				case "3":
                    List<Application> Applicant = application.GetAppliedjob();
					if (Applicant != null)
					{
						list.Print(Applicant);
						ShowProviderMenu();
					}
					else
					{
						Console.WriteLine("No Applicants");
					}
                    break;
				case "4":
					Logout();
